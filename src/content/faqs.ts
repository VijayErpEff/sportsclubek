export const HOMEPAGE_FAQS = [
  {
    question: "What sports can I play at LevelUP Sports in Elkton, MD?",
    answer:
      "LevelUP offers four sports under one roof: baseball (4 indoor batting cages with machines up to 90 MPH), cricket (full-length nets with professional bowling machines), badminton (3 BWF-standard competition courts), and pickleball (2 dedicated indoor courts). We also run a Kids Agility program for ages 5-12 that builds coordination, speed, and confidence through structured athletic training.",
  },
  {
    question: "Do I need a membership to use LevelUP Sports?",
    answer:
      "No — drop-ins are welcome! You can rent a batting cage or court on a per-session basis without any membership. Cages start at $35/hour and courts start at $25/hour. That said, memberships save you significant money if you come regularly. Our Basic plan is $49/month for unlimited court access.",
  },
  {
    question: "Can my child try a free session before signing up?",
    answer:
      "Yes! We offer a completely free trial session for every sport — no credit card required, no obligation. Your child gets a 60-minute coached session with all equipment provided, plus a personalized skill assessment. Visit our Free Trial page to book: levelupsports.us/free-trial",
  },
  {
    question: "How much does it cost to rent a batting cage or court at LevelUP?",
    answer:
      "Batting cages range from $35-45/hour depending on the cage (our pro cage with video analysis is $45). Cricket net sessions are $40/hour. Badminton courts are $30/hour. Pickleball courts are $25/hour. All prices are per court/cage, not per person — so you can split the cost with friends or family. Memberships offer unlimited access starting at $49/month.",
  },
  {
    question: "What ages are your youth sports programs for?",
    answer:
      "Our programs cover ages 5 through adult. Kids Agility is for ages 5-12. Baseball Academy runs ages 8-18 in three tiers (Foundations 8-12, Development 13-15, Advanced 16-18). Cricket Academy accepts ages 8 through adult. Badminton Academy starts at age 7 through adult. Pickleball lessons are available for all ages, though most participants are adults.",
  },
  {
    question: "Is LevelUP Sports near Middletown, Delaware?",
    answer:
      "Yes — LevelUP Sports is just 15 minutes from Middletown, DE via Route 40. We're located at 701 E Pulaski Hwy, Elkton, MD 21921. Drive times from nearby areas: Middletown, DE (15 min), Newark, DE (20 min), Bear, DE (25 min), Glasgow, DE (20 min), Wilmington, DE (30 min), North East, MD (10 min), Kennett Square, PA (25 min). We're right off I-95, exit 109A.",
  },
  {
    question: "What are LevelUP Sports' hours of operation?",
    answer:
      "We're open Monday through Saturday from 6:00 AM to 11:00 PM, and Sunday from 6:00 AM to 9:00 PM. We're open 7 days a week, 362 days a year (closed Thanksgiving, Christmas, and New Year's Day). Holiday hours may vary — check our social media or call (443) 406-6494 for holiday schedules.",
  },
  {
    question: "Where exactly is LevelUP Sports located?",
    answer:
      "We're at 701 E Pulaski Hwy (Route 40), Elkton, MD 21921. Right off I-95 exit 109A, on the main commercial corridor in Elkton. Free parking on-site with plenty of spaces. We're at the crossroads of Maryland, Delaware, and Pennsylvania — serving Cecil County, New Castle County, and Chester County families.",
  },
  {
    question: "How do I book a session at LevelUP Sports?",
    answer:
      "Three ways: (1) Book online through our website — pick your sport, choose a time, and confirm in under 60 seconds. (2) Call us at (443) 406-6494. (3) Walk in — subject to availability, but we almost always have space on weekday mornings and afternoons. We recommend booking in advance for peak hours (evenings and weekends).",
  },
  {
    question: "What is the cancellation policy at LevelUP Sports?",
    answer:
      "Court and cage rentals can be cancelled up to 24 hours in advance for a full refund. Cancellations within 24 hours can be rescheduled to another available slot but are not refundable. Memberships can be cancelled anytime with 30 days' notice — no cancellation fees, no contracts. We also allow membership freezes for up to 2 months per year at no cost.",
  },
];

export const MEMBERSHIP_FAQS = [
  {
    question: "Can I cancel my membership at any time?",
    answer:
      "Yes. All memberships can be cancelled with 30 days' notice. No long-term contracts, no cancellation fees. We want you here because you love it, not because you're locked in.",
  },
  {
    question: "Can I freeze my membership temporarily?",
    answer:
      "Absolutely. Members can freeze for up to 2 months per year at no cost. Perfect for vacations, injuries, or busy seasons. Just let us know 7 days in advance.",
  },
  {
    question: "Do family memberships require everyone to live at the same address?",
    answer:
      "Family memberships cover up to 2 adults and 3 children (under 18) at the same household. All family members get full facility access and member benefits.",
  },
  {
    question: "Is there a student or military discount?",
    answer:
      "Yes. 15% off for students with a valid ID, 20% off for active military and veterans. Discounts apply to any membership tier.",
  },
  {
    question: "What payment methods do you accept?",
    answer:
      "All major credit and debit cards, Apple Pay, Google Pay, and cash for in-person payments. Memberships are billed monthly to your card on file.",
  },
  {
    question: "Is a LevelUP membership worth it?",
    answer:
      "If you visit 3+ times per week, membership pays for itself in the first month. A single batting cage rental is $35. A single court booking is $25-30. With a Pro membership at $89/month, unlimited access works out to less than $3 per visit. Plus you get priority booking, guest passes, and academy discounts. Most members tell us they wish they'd signed up sooner.",
  },
];
