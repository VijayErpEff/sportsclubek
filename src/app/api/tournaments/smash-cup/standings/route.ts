...
export async function GET() {
  ...
  try {
    const data = await redis.get(STANDINGS_KEY);
    return NextResponse.json(data ?? null, { headers: NO_CACHE_HEADERS });
  } catch {
    return NextResponse.json(null, { status: 500, headers: NO_CACHE_HEADERS });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await redis.set(STANDINGS_KEY, JSON.stringify(state));
